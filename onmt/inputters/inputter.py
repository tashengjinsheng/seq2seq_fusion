...
def get_fields(
    ...
    if n_cue_feats is None:
        n_cue_feats = n_src_feats
    fields = {}
    ...
    cue_field_kwargs = {"n_feats": n_cue_feats,
                        "include_lengths": True,
                        "pad": pad, "bos":None, "eos": None,
                        "truncate": cue_truncate,
                        "sequential": True,
                        "base_name": "cue"}
    fields["cue"] = fields_getters["text"](**cue_field_kwargs)
    ...
    if dynamic_dict:
        ...
        # cue_map = Field(use_vocab=False,
        #                 dtype=torch.float,
        #                 postprocessing=make_cue, sequential=False)
...
def filter_example(ex, use_src_len=True, use_tgt_len=True,
                   ...
                   min_cue_len=1, max_cue_len=float('inf'),
...
def _build_fields_vocab(fields, counters, data_type, share_vocab,
                        ...
                        cue_vocab_size, cue_words_min_frequency,
                        tgt_vocab_size, tgt_words_min_frequency):
    build_fv_args = defaultdict(dict)
    build_fv_args["src"] = dict(
        max_size=src_vocab_size, min_freq=src_words_min_frequency)
    build_fv_args['cue'] = dict(
        max_size=cue_vocab_size, min_freq=cue_words_min_frequency)
    ...
    if data_type == 'text':
        ...
        if share_vocab:
            ...
            logger.info(" * merging src, cue and tgt vocab...")
            src_field = src_multifield.base_field
            cue_field = cue_multifield.base_field
            tgt_field = tgt_multifield.base_field
            _merge_field_vocabs(
                src_field, cue_field, tgt_field, vocab_size=src_vocab_size,
...
def _merge_field_vocabs(src_field, cue_field, tgt_field, vocab_size, min_freq,
                        vocab_size_multiple):
    ...
    merged = sum(
        [src_field.vocab.freqs, cue_field.vocab.freqs, tgt_field.vocab.freqs], Counter()
    )
    merged_vocab = Vocab(
    ...
    cue_field.vocab = merged_vocab
    tgt_field.vocab = merged_vocab
    assert len(src_field.vocab) == len(tgt_field.vocab) and len(src_field.vocab == cue_field.vocab)
