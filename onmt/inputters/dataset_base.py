...
def _dynamic_dict(example, src_field, tgt_field):
    ...
    cue = []
    if 'cue' in example:
        for c in example['cue']:
            cue.append(src_field.tokenize(c))
    flatten_cue = [[item for item in seq] for seq in cue]
    # make a small vocab containing just the tokens in the source sequence
    unk = src_field.unk_token
    pad = src_field.pad_token
    src_ex_vocab = Vocab(Counter(src+flatten_cue), specials=[unk, pad])
    unk_idx = src_ex_vocab.stoi[unk]
    # Map source tokens to indices in the dynamic dict.
    src_map = torch.LongTensor([src_ex_vocab.stoi[w] for w in src])
    cue_map = torch.LongTensor([[src_ex_vocab.stoi[w] for w in seq] for seq in cue])
    example["cue_map"] = cue_map
