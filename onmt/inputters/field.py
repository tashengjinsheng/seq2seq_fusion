...
class Field(RawField):
    ...
    def pad(self, minibatch):
        ...
        padded, lengths = [], []
        if isinstance(minibatch[0],list) and isinstance(minibatch[0][0], list):
            max_nfacts = max(len(x) for x in minibatch)
            max_src_len = max([max([len(x) for x in ex]) for ex in minibatch])
            for x in minibatch:
                padded_facts = []
                fact_lengths = []
                for fact in x:
                    if self.pad_first:
                        padded_facts.append(
                            [self.pad_token] * max(0, max_src_len - len(fact))
                            + ([] if self.init_token is None else [self.init_token])
                            + (list(fact[-max_src_len:] if self.truncate_first else fact[:max_src_len]))
                    ...
                    fact_lengths.append(len(padded_facts[-1]) - max(0,max_src_len - len(fact)))
                for _ in range(max_nfacts - len(padded_facts)):
                    padded_facts.append([self.pad_token] * len(padded_facts[-1]))
                    fact_lengths.append(0)
                padded.append(padded_facts)
                lengths.append(fact_lengths)
    ...
    def numericalize(self, arr, device=None):
        ...
        if self.use_vocab:
            ...
            if self.sequential:
                ...
                elif isinstance(arr[0], six.string_types):
                    arr = [self.voab.stoi[x] for x in arr]
                elif isinstance(arr[0][0], six.string_types):
                    arr = [[self.vocab.stoi[x] for x in ex] for ex in arr]
                elif isinstance(arr[0][0][0], six.string_types):
                    arr = [[[self.vocab.stoi[x] for x in ex] for ex in eex] for eex in arr]
            else:
            ...
            if not self.sequential:
                ...
                if not isinstance(arr,list):
                    arr = numericalization_func(arr)
                elif not isinstance(arr[0],list):
                    arr = [numericalization_func(x) if isinstance(x, six.string_types)
                       else x for x in arr]
                elif not isinstance(arr[0][0],list):
                    arr = [[numericalization_func(x) if isinstance(x,six.string_types) else x for x in ex] for ex in arr]
            if self.postprocessing is not None:
        ...
        if self.sequential and not self.batch_first:
            if var.dim() == 2:
                ...
                var = var.transpose(0,2)
            else:
                raise ValueError("var dim not equal to 2 or 3")
        if self.sequential:
            var = var.contiguous()
